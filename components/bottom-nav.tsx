...
export function BottomNav() {
  ...
  return (
    <div
      className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[390px] glass border-t border-[var(--border)] z-50"
  ...
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path
        ...
        stroke={active ? '#FF6B6B' : 'var(--icon-inactive)'}
        strokeWidth="1.8"
  ...
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path
        ...
        fill={active ? '#FF6B6B' : 'var(--icon-inactive)'}
      />
    </svg>
  )
}
function StatsIcon({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <rect x="4" y="14" width="4" height="6" rx="1" fill={active ? '#FF6B6B' : 'var(--icon-inactive)'} />
      <rect x="10" y="9" width="4" height="11" rx="1" fill={active ? '#FF6B6B' : 'var(--icon-inactive)'} opacity={active ? 1 : 0.5} />
      <rect x="16" y="4" width="4" height="16" rx="1" fill={active ? '#FF6B6B' : 'var(--icon-inactive)'} opacity={active ? 1 : 0.3} />
    </svg>
  )
}
function ProfileIcon({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <circle cx="12" cy="8" r="4" stroke={active ? '#FF6B6B' : 'var(--icon-inactive)'} strokeWidth="1.8" />
      <path
